...
def test_products_list(session):
    ...
    assert len(data) >= 6  # catalog grew — check minimum, not exact
    names = {p["name"] for p in data}
    expected_subset = {"Mikaté Sucré", "Mikaté Salé", "Jus de Bissap", "Jus de Gingembre"}
    assert expected_subset.issubset(names)
    for p in data:
        assert {"id", "name", "category", "description", "image_url", "options"} <= set(p.keys())
        assert isinstance(p["options"], list) and len(p["options"]) >= 1
...
def test_list_orders_contains_created(session):
    # GET /api/orders is admin-protected now
    r = session.get(f"{API}/orders", headers={"X-Admin-Password": "mikate2025"})
    assert r.status_code == 200
    data = r.json()
    assert isinstance(data, list)
    assert len(data) >= 1
